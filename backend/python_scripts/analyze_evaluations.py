...
def main():
    ...
    if not evaluations_list:
        print(json.dumps({"suggestions": [], "averages_by_question": {}, "analysis_by_question": {}}))
        return
    ...
    df.dropna(subset=[f'nota_{list(CATEGORIES.keys())[0]}'], inplace=True)
    ...
    for key, name in CATEGORIES.items():
        ...
        avg_score = df[nota_col].mean()
        averages_by_question[nota_col] = round(avg_score, 2)

        score_distribution = df[nota_col].value_counts().to_dict()
        comments = df[comentario_col].dropna().tolist()
        sentiment_score = df[comentario_col].apply(analyze_sentiment).mean()

        suggestion = generate_suggestion(name, avg_score, sentiment_score)
...
def generate_suggestion(name, avg_score, sentiment_score):
    if avg_score < 2.5 and sentiment_score < -0.5:
        return f"**Ponto Crítico em {name}:** A média ({round(avg_score, 2)}) é muito baixa e o sentimento é fortemente negativo. É urgente investigar as causas e implementar um plano de ação corretivo imediato. Priorize a escuta ativa dos alunos para entender os problemas específicos."
    elif avg_score < 3.5 and sentiment_score < 0:
        return f"**Ponto de Atenção em {name}:** A média ({round(avg_score, 2)}) é baixa e o sentimento é negativo. Recomenda-se focar em melhorias nesta área. Analise os comentários para identificar os pontos fracos e planeje ações de médio prazo."
    elif avg_score > 4.0 and sentiment_score > 0.5:
        return f"**Ponto Forte em {name}:** A média ({round(avg_score, 2)}) é alta e o sentimento é fortemente positivo. Mantenha e promova as boas práticas desta área. Considere usar este sucesso como modelo para outras áreas."
    elif avg_score > 3.5 and sentiment_score >= 0:
        return f"**Ponto Positivo em {name}:** A média ({round(avg_score, 2)}) é boa e o sentimento é neutro a positivo. Continue monitorando e buscando pequenas melhorias para manter a qualidade."
    return ""
